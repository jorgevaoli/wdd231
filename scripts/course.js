document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayCourses(coursesToDisplay) {
        ...
        coursesToDisplay.forEach(course => {
            ...
            courseHeader.innerHTML = `${course.subject} ${course.number} - ${course.title}`;

            if (course.completed) {
                const checkMark = document.createElement('span');
                checkMark.className = 'check-mark';
                checkMark.innerHTML = '✓';
                courseHeader.appendChild(checkMark);
            }

            const courseInfo = document.createElement('div');
            courseInfo.className = 'course-info';
            courseInfo.textContent = `${course.credits} credits | ${course.certificate}`;

            const courseDescription = document.createElement('div');
            courseDescription.className = 'course-description';
            courseDescription.textContent = course.description;

            const courseTech = document.createElement('div');
            courseTech.className = 'course-tech';

            course.technology.forEach(tech => {
                const techTag = document.createElement('span');
                techTag.className = 'tech-tag';
                techTag.textContent = tech;
                courseTech.appendChild(techTag);
            });

            courseCard.appendChild(courseHeader);
            courseCard.appendChild(courseInfo);
            courseCard.appendChild(courseDescription);
            courseCard.appendChild(courseTech);
